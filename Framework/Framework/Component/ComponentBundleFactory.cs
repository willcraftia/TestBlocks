...
{
    public sealed class ComponentBundleFactory
    {
        #region Holder
        ...
        public ComponentBundleFactory() { }

        public ComponentBundleFactory(ITypeRegistory typeRegistory)
        {
        ...
        public void Initialize(ref BundleDefinition definition)
        {
            if (ArrayHelper.IsNullOrEmpty(definition.Entries)) return;

            for (int i = 0; i < definition.Entries.Length; i++)
                AddComponentDefinition(ref definition.Entries[i]);
        }

        void AddComponentDefinition(ref BundleEntryDefinition definition)
        {
